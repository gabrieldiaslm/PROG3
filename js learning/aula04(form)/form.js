...
const formSubmit = (event) => {
  ...
  if (studentId.value.length != 6) {
    studentId.nextElementSibling.textContent =
      "A matrícula deve possuir 6 caracteres";
    validForm = false;
  }

  const studentName = form["student-name"];
  if (studentName.value.length < 10) {
    studentName.nextElementSibling.textContent =
      "O nome deve possuir, no mínimo, 10 caracteres";
    validForm = false;
  }
